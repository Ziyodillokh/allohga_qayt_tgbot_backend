...
export class UploadService {
  ...
  getFilePath(
    filename: string,
    type: "avatars" | "attachments" = "attachments",
  ): string {
  ...
  async downloadAndSaveAvatar(
    ...
    try {
      ...
      if (!response.ok) {
        console.error(
          "[UploadService] Failed to download avatar:",
          response.status,
        );
        return null;
      }
  ...
  private validateFile(file: Express.Multer.File) {
    ...
    if (file.size > this.maxFileSize) {
      throw new BadRequestException("Fayl hajmi 50MB dan oshmasligi kerek");
    }
  ...
  private validateAttachment(file: Express.Multer.File) {
    ...
    if (!allowedExtensions.includes(ext)) {
      throw new BadRequestException("Fayl formati qollab-quvvatlanmaydi");
    }
    if (!file.buffer || file.buffer.length === 0) {
      throw new BadRequestException("Fayl buffer bosh");
    }
  }
}
